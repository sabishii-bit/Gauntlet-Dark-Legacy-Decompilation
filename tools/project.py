...
from .gdl.build_provenance import RecordingWriter
...
def generate_build_ninja(
    ...
    if build_config:
        ...
        def add_unit(build_obj: BuildConfigUnit, link_step: LinkStep):
            ...
            provenance = {
                "name": obj_name, "module": link_step.name,
                "autogenerated": build_obj["autogenerated"],
                "code_size": build_obj.get("code_size", 0),
                "data_size": build_obj.get("data_size", 0),
                "configured": obj is not None,
                "configured_completed": bool(obj and obj.completed),
                "extracted_object": str(obj_path).replace("\\", "/") if obj_path else None,
                "source_object": None, "linked_object": None, "linkage": "missing",
            }
            n.units.append(provenance)
            if obj is None:
                ...
                if obj_path is not None:
                    ...
                    provenance.update(linked_object=str(obj_path).replace("\\", "/"), linkage="extracted_fallback")
                return
            ...
            if link_built_obj and built_obj_path is not None:
                ...
                provenance.update(linked_object=str(built_obj_path).replace("\\", "/"), linkage="source")
            elif obj_path is not None:
                # Use the original (extracted) object
                link_step.add(Path(obj_path))
                provenance.update(linked_object=str(obj_path).replace("\\", "/"), linkage="extracted_fallback")
            provenance["source_object"] = str(built_obj_path).replace("\\", "/") if built_obj_path else None
    ...
    n.build(
        ...
        implicit=[
            ...
            python_lib_dir / "gdl" / "build_provenance.py",
...
def generate_objdiff_config(
    ...
    COMPILER_MAP = {
        ...
        # The local scheduler derivatives are not hosted by decomp.me.  Use
        # each one's exact upstream base for scratch experiments; local builds
        # remain SHA-pinned to the derived executable by configure.py.
        "GC/1.2.5s": "mwcc_233_163",
        "GC/1.2.5sn": "mwcc_233_163n",
        "GC/1.3": "mwcc_242_53",
