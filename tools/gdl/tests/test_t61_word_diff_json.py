...
Re-verified on the native-only tree at a7ec65c95: movieplayer still reports
52 rows and exactly one count-asymmetric row, now summing to 648 open
differing words, and `--unit` resolves the DIRECT compiler object ("active
compiler-stage output (raw)") -- there is no .postprocess body to prefer any
more, and `pinned` is 0 for every row because no rule configuration exists.
...
class LiveJson(unittest.TestCase):
    ...
    def test_the_unit_mode_reads_the_direct_compiler_object(self):
        # Run 62, native-only: there is no .postprocess body object to
        # resolve any more, so the unit mode must name the compile edge's
        # own output and say so, not fall back to a phantom path or refuse.
        self.assertIn("raw", self.data["object"])
        self.assertNotIn("postprocess", self.data["object"])
        self.assertEqual(self.data["totals"]["pinned"], 0)
