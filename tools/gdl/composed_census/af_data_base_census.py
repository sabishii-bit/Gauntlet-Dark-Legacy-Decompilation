...
def census(unit):
    ...
    for sec in DATA_SECTIONS:
        ...
        # single-candidate rule stands, labelled for what it is. Measured
        # over the 269 (unit, section) pairs that resolve any base at
        # 9efe90cb5 (build/t4_scratch/t4_af_rule_delta.py): 5 paste lines
        # GAINED where the DOL elected one of several candidates
        # (controls/movieplayer .sdata2, attract/auxscreen .sdata, SIBios
        # .data), 5 WITHDRAWN where the single candidate is measurably wrong
        # (dcs .rodata 95.6%, bosscam .sdata2 50.0%, gauntworld .rodata
        # 11.9%, combat .rodata 10.7%, nubinit .rodata 0.0%), 259 unchanged.
        # Requiring byte equality even where nothing is comparable would have
        # withdrawn 26 MORE, including landed correct claims such as
        # zlib/inflate .data whose every word is relocated -- which is why
