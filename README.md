...
| `datadiff.py` | Pre-link byte comparison of a unit's emitted data sections (`.rodata .data .sdata .sdata2`) against the original DOL at the claimed addresses; words carrying relocations are skipped but counted. Catches what `fndiff` structurally normalizes away: wrong constant *values* (the mathfunc 1e-5-vs-1e-14 epsilon), wrong pool/table emission *order*, initializer typos. `--matching` sweeps every Matching unit; run it before `finish_tu` on any TU with nontrivial data. |
| `fnsurvey.py` | Region survey for shape-mapping campaigns: for every function in a VA range prints size, callees (REL24), and data refs (SDA21/ADDR16), with `--heads N` for opening instructions and `--calls X` to filter by callee. One call replaces a fnasm-per-function mapping session (built for the AUDIO region; its full output lives in `research/audio_region_survey.txt`). |
