...
/* lint-begin FM007: shipped audio data, final form.  Every entry equals round(0xFF64 * 10 ** (-(964 - i) * 0.1 / 20)) to within 1 LSB -- a 0.1 dB per step linear-amplitude table over 96.4 dB plus a 0x0000 terminator.  Its length is confirmed by sndDbToMix, which indexes sndDbTable[db + 0x388] after clamping db at 0x3C, and 0x388 + 0x3C + 1 == 965. */
u16 sndDbTable[966] = {
...
/* lint-begin FM007: shipped audio data, final form.  A 50-step 0x00..0xFF ramp handed to AISetStreamVolLeft/AISetStreamVolRight, whose parameter is a u8 hardware volume. */
