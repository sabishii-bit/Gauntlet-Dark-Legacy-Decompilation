/* GLUE.OBJ: model loading and OBJGRP world-matrix helpers shared by the
 * world runtime and the item system (.text 0x8005A1EC..0x8005A738). */
#include "types.h"
...
s32 fn_8005A1EC(const char* name, void** outData)
{
    ...
    if (outData != 0) {
        if (FileExists(name, "anim")) {
            *outData = AllocMem(FileSize(name, "anim"));
        } else {
            *outData = 0;
        }
    }
