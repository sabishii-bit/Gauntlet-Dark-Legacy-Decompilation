...
void move_logic01(s32 index)
{
    ...
    dead0 = *(s32*)(row01 + OFF_E(dead_end));
    e0 = row01 + 3608;
    e = (Enemy*)(u8*)e0;
    if (dead0 > 0) {
        stuck = 1;
...
void move_logic30(s32 index)
{
    ...
    if (it < 0) {
        ...
        if (*(s32*)(op + OFF_E(state)) != 1) {
            flee = 0;
        } else if (*(f32*)(op + OFF_E(actual_dist)) > e->sight) {
            flee = 0;
        } else if (index == it || e->birth_style != 0 || e->dead_end > 0) {
            goto flee_zero30;
        } else {
...
placed:
    ...
    if (e->actionlist[1].animidx >= 0) {
        InitAnim(0.0f, &e->atree.animinfo,
