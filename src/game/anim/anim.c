...
u32 AnimateTree(f32 time, animinfo* info, s32 seq, s32 frame, s32 mode)
{
    ...
    if (restart) {
        ...
        if (!done ||
            (f64)info->starttime < (f64)info->atime - 0.0333333333) {
            info->starttime = (f32)((f64)info->atime - 0.0333333333);
        }
...
s32 InitAnim(f32 time, animinfo* info, s32 seq, s32 frame, s32 active)
{
    ...
    rep = *(s16*)(s + 0x24);
    if (*(s16*)(s + 0x22) > 0) {
        info->seqscale = (f32)((f64)*(s16*)(s + 0x22) * 0.0333333333 *
                               (f64)info->animscale);
    } else {
    ...
    if (time != lbl_803457B4) {
        info->starttime =
            (f32)((f64)-(info->frame * sc - info->atime) - 0.0333333333);
        info->transtime =
            (f32)((f64)(info->atime + time) - 0.0333333333);
    } else {
                ...
                f32* posoff)
{
    ...
    flagRaw = key[0];
    bytes = (u8*)&flagRaw;
    flags = (u16)(bytes[0] | (bytes[1] << 8));
    nextRaw = key[1];
    bytes = (u8*)&nextRaw;
    next = (u16)(bytes[0] | (bytes[1] << 8));
    if (info->setpanim != 0) {
        *(s16*)((u8*)node + 8) = -1;
        *(u16*)((u8*)node + 10) = 0;
        info->setpanim = 2;
