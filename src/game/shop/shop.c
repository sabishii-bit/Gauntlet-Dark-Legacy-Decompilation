...
extern struct PadStateView PlayerControl[];
...
extern PadStateView PlayerControl[]; /* pad states, stride 60, buttons @+8 (CTL in controls.c) */
extern void AudioCursorSelect(void);
...
static s32 shop_show_final_stats(u8* pl)
{
    ...
    if (done != 0) {
        if ((PlayerControl[*(s32*)pl].buttons & 0x2000000) != 0) {
            AudioCursorSelect();
            *(s32*)(pl + offsetof(Player, field_A6C)) = 0;
            return 1;
        }
...
static s32 shop_show_lv(u8* pl, s32 final)
{
    ...
    if (done != 0) {
        if (PlayerControl[*(s32*)pl].buttons & 0x2000000) {
            AudioCursorSelect();
            *(s32*)(pl + offsetof(Player, field_A6C)) = 0;
            return 1;
        }
...
void fn_8009A0AC(s32 col)
{
    ...
    goldRaw = *(s32*)(pl + offsetof(Player, gold)) -
              *(s32*)((b240 = pl + cls * 240) +
                      (offsetof(Player, save_backup.stuff) + offsetof(PlayerCharSave, gold)));
...
static s32 write_shop_menu(s32 player, s32 scroll)
{
    ...
    *(scrollflag += 12) = 0;
    ...
    for (j = 0; j < lbl_80344C10; j++, joff += 4, itemoff += 80) {
        ...
        {
            s32 rawPrice = *(s32*)(item + offsetof(DSItem, price));
            if (rawPrice > 0) {
                s32 x;
                s32 ytxt;
                u32 msg;
