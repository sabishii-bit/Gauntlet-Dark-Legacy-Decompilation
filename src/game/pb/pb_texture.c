...
/* ------------------------------------------------------------------ */
/* Cache maintenance used before GX consumes palette memory. */
/* ------------------------------------------------------------------ */
...
/* TLUT region manager: 0x30-byte header, then GXTlutRegion[] at +0x30,
...
typedef struct PbTlutMgrView {
    ...
    GXTlutRegion regions[0x2f]; /* 0x030 : 16-byte SDK region objects */
    ...
    u8 _5aa[2];              /* 0x5aa */
    PbSpecialTlut special[2];/* 0x5ac : 4-bit and 8-bit fallback TLUTs */
} PbTlutMgrView;
extern PbTlutMgrView lbl_802C7438; /* TLUT manager base (0x802C7438) */
extern u8 lbl_802C7468[];   /* &mgr.regions[0]     (0x802C7468) */

/* GC ROM texture record and its corresponding 0x30-byte texture entry.
 * GX calls select the 32-byte GXTexObj at +0 and 12-byte GXTlutObj at +0x20;
 * signed region selectors follow at +0x2c/+0x2d. These enclosing view names
 * are local reconstruction names, not recovered SDK or Xbox declarations. */
...
typedef struct PbTextureDescView {
    u8 _pad00[0x48];
    u32 textureCount;             /* 0x48 */
    u8 _pad4c[0x0c];
    PbRomTexture* romTextures;    /* 0x58 */
    u8 _pad5c[0x14];
    s32 dataShift;                /* 0x70 */
    u8 _pad74[4];
    u8* usedFlags;                /* 0x78 */
    u16 lightmapBase;             /* 0x7c */
    u16 lightmapCount;            /* 0x7e */
    PbTextureObject* texObjects;  /* 0x80 */
} PbTextureDescView;
...
/* GX callback: map a tlut_name to its GXTlutRegion (stride 0x10). */
static GXTlutRegion* sTlutRegionCallback(u32 name);

/* Initialise all hardware TLUT regions and register the region callback.
 * The byte cursor is retained provisionally: with the SDK object types,
 * direct region-member and loop-local pointer forms changed the native
 * residual from 4 to 6/8 words; a separate array base removed an instruction
 * (2026-09-14). The original address-expression grouping is still unresolved. */
void pbInitTlutRegions(void) {
    ...
    for (i = lbl_80348FC8[0]; i < lbl_80348FD0[0]; i++) {
        slot = mgr + i * 0x10;
        GXInitTlutRegion((GXTlutRegion*)(slot + 0x30), addr, 1);
        addr += 0x200;
    }
    for (; i < lbl_80348FD0[1]; i++) {
        slot = mgr + i * 0x10;
        GXInitTlutRegion((GXTlutRegion*)(slot + 0x30), addr, 0x10);
        addr += 0x2000;
    }
...
static GXTlutRegion* sTlutRegionCallback(u32 name) {
    return (GXTlutRegion*)(lbl_802C7468 + name * 0x10);
}
 ...
 * Returns the region index (a texture handle). GCN-only. The target bounds
 * are [0,16) and [16,47), so each residency/lock bit is an unsigned 64-bit
 * shift. MWCC emits its own __shl2i call for this operator. */
int fn_800C6BB4(u8 sizeClass, s32 handle) {
    ...
    if ((lbl_80116AB0[sizeClass] & held) != lbl_80116AB0[sizeClass]) {
        s32 z = 0;
        while (first < last) {
            if ((held & (1ULL << first)) == z)
                break;
            first++;
        }
        {
            u8* q;
            lbl_803450D8 |= (1ULL << first);
            q = mgr + first * 4;
            mgr[first] = z;
...
void pbSetupTextures(s32 id) {
    u8* lookup;
    PbTextureDescView* desc;
    PbTextureObject* obj;
    s32 i;
    u32 width;
    u32 height;
    u8* data;
    PbTextureObject* texObjs;
    s32 bitsPerPixel;
    s32 texOffset;
    s32 objOffset;
    s32 formatOffset;
    ...
    texOffset = 0;
    objOffset = 0;
    desc = (PbTextureDescView*)((TEXDESCENT*)gWinGlobals->tbl)[id].desc;
    texObjs = desc->texObjects;

    while ((u32)i < desc->textureCount) {
        PbRomTexture* tex;
        ...
        obj = (PbTextureObject*)((u8*)texObjs + objOffset);
        obj->region = -1;
        found = 0;
        obj->paletteRegion = -1;
        tex = (PbRomTexture*)((u8*)(texBase = desc->romTextures) +
                              texOffset);
            ...
            if (!found) {
                ...
                if ((type & 8) != 0) {
                    ...
                    if (type > 0) {
                        ...
                        GXInitTlutObj(&obj->tlutObject, data, 2,
                                     (u16)paletteEntries);
                        {
                            u8* entry = lookup;
                            entry += formatOffset;
                ...
                if (bitsPerPixel <= 8) {
                    ...
                    GXInitTexObjCI(&obj->object, data, (u16)width, (u16)height,
                                   lbl_80348FD8[paletteClass], 1, 1, 0,
                                   lbl_80348FE0[paletteClass]);
                } else {
                    GXInitTexObj(&obj->object, data, (u16)width, (u16)height, 5,
                                 1, 1, 0);
                }
                GXInitTexObjUserData(&obj->object, (void*)-1);
            }
        }
        i++;
        texOffset += 0x10;
...
void pbResetTextures(void) {
    ...
    for (i = 0; i < *(s32*)g->tbl; i++) {
        ...
        u8** pp = (u8**)(e + 0x4);
        if (*(s32*)(e + 0x10) == 0) {
            PbTextureDescView* desc = (PbTextureDescView*)*pp;
            memset(desc->usedFlags, 0, (desc->textureCount + 7) & ~7);
        }
    }
    lbl_80343F78 = -1;
    lbl_80343F7C = -1;
...
void fn_800C7214(s32 id) {
    ...
    PbTextureDescView* desc = *(PbTextureDescView**)e;
    PbRomTexture* t;
    s32 shift;
    s32 i;
    ...
    shift = desc->dataShift;
    for (i = 0; i < desc->textureCount; i++) {
        t = &desc->romTextures[i];
        t->index = -1;
        t->model = (s8)id;
        if (t->width < 1 || t->height < 1 || (t->flags & 0x100)) {
            t->flags |= 0x100;
            t->data = 0;
        } else {
            t->data = (void*)(shift + (s32)t->data);
        }
    }
    pbSetupTextures(id);
}
...
void fn_800C73E0(void) {
    ...
    if (lbl_80345114 == 1) {
        ...
        GXInitTlutObj(&((PbTlutMgrView*)mgr)->special[0].object,
                      (void*)lbl_803450E8, 2, 16);
        DCFlushRange((void*)lbl_803450EC, 4);
        GXInitTlutObj(&((PbTlutMgrView*)mgr)->special[1].object,
                      (void*)lbl_803450EC, 2, 0x100);
        lbl_80345114 = 2;
    }
    if (lbl_80345114 == 2) {
        s32* cur;
        s32 region;
        if (*(cur = &((PbTlutMgrView*)mgr)->special[0].handle) == -1) {
            *cur = fn_800C6BB4(0, 0xFFFF0000);
            region = *cur;
            lbl_803450E0 |= (1ULL << region);
            lbl_80345108 = lbl_80345108 + 1;
            GXLoadTlut(&((PbTlutMgrView*)mgr)->special[0].object, region);
        }
    }
    if (lbl_80345114 == 0) {
        ...
        ((PbTlutMgrView*)mgr)->special[1].handle = -1;
        ((PbTlutMgrView*)mgr)->special[0].handle = -1;
        lbl_80345114 = 1;
    }
}
    ...
    if ((format & 8) != 0) {
        ...
        if (*(s32*)(specialBase + offsetof(PbTlutMgrView, special) +
            ...
            *(s32*)(special + offsetof(PbSpecialTlut, handle)) =
            ...
            GXLoadTlut((GXTlutObj*)special, *(s32*)(special + offsetof(PbSpecialTlut, handle)));
            newLoad = 0;
        }
        if (texture->region != *(s32*)(special + offsetof(PbSpecialTlut, handle))) {
            GXInitTexObjTlut(&texture->object, *(s32*)(special + offsetof(PbSpecialTlut, handle)));
        ...
        if (texture->paletteRegion == -1) {
            ...
            GXInitTexObjTlut(&texture->object, texture->paletteRegion);
            newLoad = 0;
        }
    }
    if (newLoad != 0) {
...
int fn_800C780C(s32 id, s32 idx, u32 mask) {
    ...
    PbTextureDescView* desc = (PbTextureDescView*)t[id].desc;
    u8* flags = desc->usedFlags;
    s32 i;
    u32 bit;
